...
export class Input {
    constructor(countImages, directory, imagesFormat, startNameNumber) {
        ...
        */
        ...
        this.loadTrain = this.loadTrain.bind(this);
    }

    allClear(context, width, height) {
        context.clearRect(0, 0, width, height) //Каждый раз очищаем холст
    }

    load(ctx, width, height) {
        this.pixels = this.images.map((value, indexValue) => //Пробегаемся по всей выборке изображений
            {
                this.allClear(ctx, width, height)
                ctx.drawImage(value, 0, 0);            //Прорисовываем изображение на холсте
                let data = [];                           //Инициализируем вектор оветов изображения
                for (let x = 0; x < width; x++)      //Пробегаемся по столбикам изображения
                {
                    for (let y = 0; y < height; y++) //Пробегаемся по строкам изображения
                    {
                        let rgb = ctx.getImageData(x, y, 1, 1).data.slice(0, 3) //Получаем красный синий и зеленый цвет пикселя
    ...
    loadTrain(url, ctx, width, height) {
        this.imagesTraining.src = url;
        setTimeout(() => {
            if (url) this.allClear(ctx, width, height);
            ctx.drawImage(this.imagesTraining, 0, 0);            //Прорисовываем изображение на холсте
...
export class Network {
    constructor(x, images) {
        this.x = x
        this.images = images;
        this.w = this.x[0]?.map(line => this.x[0].map(() => 0));
        this.deference = 65;
